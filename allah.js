...
function dumpJsonToFile(json) {
    var fs = require("fs");
    fs.writeFile("dump.json", JSON.stringify(json, null, 2), function(err) {
        if (err) {
...
class Bot {
    ...
    sendPacket(packet) {
        this.socket.write(encodeJson(packet));
    }


    timeoutPacket(packet, timeout) {
        if (this.syncTimeInterval == null) {
            this.syncTimeInterval = setInterval(this.checkPacketQueue.bind(this), 100);
        }
    ...
    checkPacketValidity(data) {
        if (this.dataLength == 0) {
            ...
            this.dataLength = data.readInt32LE();
            this.receivedData = [];
        }
